...
本文档基于瑞芯微官方文档 `Rockchip_Developer_Guide_UART_CN.pdf`（V1.5.0, 2021-12-22）整理，说明 Rockchip 系列芯片 UART 的使用和调试方法，包括 UART 作为普通串口和控制台两种使用场景。

:::info 适用范围
- **芯片平台**：所有采用 Linux 3.10 / 4.4 / 4.19 内核的 Rockchip 芯片
...
:::caution 注意
实际芯片中的 UART 支持的功能请以芯片手册中 UART 章节的描述为准，部分 UART 功能会进行适当裁剪。
:::
...
在不同版本的 Linux kernel 中，UART 相关的 menuconfig 配置均在以下路径：
...
在不同版本的 Linux kernel 中，UART 的 dts 配置均与以下典型配置类似。以 Linux kernel 4.19 RK3568 芯片为例，在 `rk3568.dtsi` 中：
...
| 参数 | 可选值 | 说明 |
| :--- | :--- | :--- |
| `dma-names` | `"tx"` / `"rx"` | 打开 tx/rx dma |
| | `"!tx"` / `"!rx"` | 关闭 tx/rx dma |
| `pinctrl-0` | `&uart1m0_xfer` | 配置 tx 和 rx 引脚为 iomux group 0 |
| | `&uart1m1_xfer` | 配置 tx 和 rx 引脚为 iomux group 1 |
| | `&uart1m0_ctsn` + `&uart1m0_rtsn` | 配置硬件流控 cts/rts 引脚为 group 0 |
| | `&uart1m1_ctsn` + `&uart1m1_rtsn` | 配置硬件流控 cts/rts 引脚为 group 1 |
| `status` | `"okay"` | 打开 |
| | `"disabled"` | 关闭 |

**示例**：打开 RK3568 UART1，打开 DMA，配置硬件自动流控的 iomux 为 group0：
...
:::tip 提示
`pinctrl-0` 中对于硬件自动流控的操作仅仅是配置引脚 iomux，实际使能硬件自动流控的操作在 UART 驱动中。如果不需要使用硬件自动流控，cts 和 rts 引脚的 iomux 配置可以去掉。
:::

### 波特率配置

UART 波特率 = 工作时钟源 / 内部分频系数 / 16。

- 当工作时钟源由 24M 晶振直接提供时，UART 使用内部分频系数得到需要的波特率
- 当工作时钟源由 CRU 模块通过 PLL 分频提供时，UART 波特率一般为工作时钟源的 1/16

UART 驱动会根据配置的波特率大小自动去获取需要的工作时钟频率，可以通过以下命令查询：

```bash
cat /sys/kernel/debug/clk/clk_summary | grep uart
```

Rockchip UART 对常用的波特率确保稳定支持：115200、460800、921600、1500000、3000000、4000000 等。对于一些特殊的波特率，可能需要修改工作时钟分频策略才能支持。

### 使用 DMA

UART 使用 DMA 传输模式只有在数据量很大时才会产生较为明显的减轻 CPU 负载的效果。一般情况下，和使用中断传输模式相比，UART 使用 DMA 传输模式并不一定能提高数据传输速度，原因包括：

- 现在 CPU 性能都很高，传输瓶颈在外设
- 启动 DMA 需要消耗额外的资源
- UART 数据存在长度不确定的特性，会使 DMA 传输效率下降

**建议**：一般情况下使用默认中断传输模式，此时会有以下打印：

```
failed to request DMA, use interrupt mode
```

在 DMA 通道资源紧张的场景下，可以考虑关掉 TX 的 DMA 传输，此时会有以下打印：
...
1. UART 驱动使能硬件自动流控功能
2. dts 中已经切换 cts 和 rts 流控引脚的 iomux

**建议**：在高波特率（1.5M 波特率及以上）、大数据量的场景下都使用硬件自动流控（即使用四线 UART）。
...
:::caution 注意
串口唤醒系统需要同时修改 trust 固件，请联系 Rockchip 以获取支持。
:::
...
普通串口设备根据 dts 中的 `aliases` 来对串口进行编号，对应注册成 `ttySx` 设备：
...
如果需要把 uart3 注册成 ttyS1，可以修改 aliases：
...
Rockchip UART 作为控制台，使用 fiq_debugger 流程。Rockchip SDK 一般会将 uart2 配置为 `ttyFIQ0` 设备。
...
fiq_debugger 相关的 menuconfig 配置路径：
...
以 Linux kernel 4.19 RK3568 为例，fiq_debugger 节点配置如下。由于 fiq_debugger 和普通串口互斥，在使能 fiq_debugger 节点后必须禁用对应的普通串口 uart 节点。
...
tx_rx       : s=发送, r=接收
file_name   : 发送/接收数据的文件名
baudrate    : 波特率
flow_control: 0=关闭硬件流控, 1=开启硬件流控 (RTS/CTS)
max_delay   : 发送时每组数据之间的延迟秒数，0=连续发送
random_size : 0=固定大小, 1=随机大小（仅发送时有效）
```
...
使用 PC 端串口调试工具发送数据，测试程序将自动检测。检测到 U（0x55）接收正确，检测到其它字符将打印 16 进制 ASCII 码值。
...
- 原始文档：`Rockchip_Developer_Guide_UART_CN.pdf` V1.5.0
